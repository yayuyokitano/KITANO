export function deckContent(deckName:string, deckFile:string, deckID:string, numberRev:string, numberNew:string) {
  return `<a class="gotoDeck" deckfile="${deckFile}">
    ...
    <div class="deckBtnList">
      <button class="deckEdit deckBtn" onclick="callbackList.editCards(this)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black" width="18px" height="18px"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM21.41 6.34l-3.75-3.75-2.53 2.54 3.75 3.75 2.53-2.54z"/></svg></button>
...
export function editDeckMap (target:any) {
  return {
    Main: `<h2 deckid="${target.id}" deckfile="${target.path}">${target.name}</h2><label>Deck name: <input type="text" id="deckEditNameInput" onchange="listeners.setPopupChange(this); callbackList.updateDeckSettings(this, ['name'])" value="${target.name}"></label><br><input id="useDefaultNewCards" type="checkbox" onchange="listeners.setPopupChange(this); callbackList.updateDeckSettings(this, ['settings', 'newPerDay']);" class="hider"><label for="useDefaultNewCards"> Use default number of new cards per day</label><label>New cards per day: <input type="number" id="newCardsPerDay" onchange="listeners.setPopupChange(this); callbackList.updateDeckSettings(this, ['settings', 'newPerDay'])"></label><br><input id="useDefaultMaxCards" type="checkbox" class="hider" onchange="listeners.setPopupChange(this); callbackList.updateDeckSettings(this, ['settings', 'revPerDay'])"><label for="useDefaultMaxCards"> Use default max number of cards per day</label><label>Max number of cards per day: <input type="number" id="maxCardsPerDay" onchange="listeners.setPopupChange(this); callbackList.updateDeckSettings(this, ['settings', 'revPerDay'])"></label>`
  }
}

export function editCardMap (target:any) {
  return {
    Notes: `<h2 id="cardMenuPrimary" deckid="${target.id}" deckfile="${target.path}">Notes</h2><div id="noteDetails"></div>`,
    Cards: `<h2>Cards</h2><div id="cardDetails"></div>`,
  }
}
...
export const deckEditor = "<ul id='editorSidebar' class='popupSetting'></ul><div id='editorContent' class='popupContent'><div id='editorSearch' style='display: none;'><label>Search: <input type='text' oninput='listeners.editorSearch(this.value)'></label></div><div id='editorDisplay'></div></div><div id='strongPopupDetail'></div>";
